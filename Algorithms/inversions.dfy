...
module Inversions {
    ...
    lemma {:verify } {:vcs_split_on_every_assert} something2(xs: seq<int>, ys: seq<int>, sxs: seq<int>, sys: seq<int>, k: int)
        requires xs != []
        requires 0 <= k < |xs|
        ...
        ensures |JoinedWith2(sxs, sys, 0)| == |JoinedWith2(xs, ys, k)|
    {
        if ys == [] {
            ...
            if IsInverted(xs+ys, k, |xs|) {
                ...
                something2(xs, ys[1..], sxs, sys[0..j] + sys[j+1..], k);
    ...
    {
        something2(xs, ys, sxs, sys, k);
        JoinedWithSame(sxs, sys, 0);
        JoinedWithSame(xs, ys, k);

    }

    lemma {:verify } {:vcs_split_on_every_assert} inversetConcatOrderedUnorderedSame(xs: seq<int>, ys: seq<int>, sxs: seq<int>, sys: seq<int>)
        requires multiset(xs) == multiset(sxs)
        requires multiset(ys) == multiset(sys)
        requires Sorted(sxs)
        requires Sorted(sys)
